...
//    };
   ...
   .then(data => {
    ...
    //    let name = data.results[0].openfda.brand_name;
    //    let generic = data.results[0].openfda.generic_name;
    //    let route = data.results[0].openfda.route;
       //console.log(name, generic, route);
       for (let i = 0; i < searchMeds.length; i += 1) {
            searchResults += generateHtmlTemplate(searchMeds[i], i);
       }
       //return (name, generic, route);
       console.log(data.results);
       console.log(searchResults);
       resultsDisplay.prepend(searchResults);
...
function generateHtmlTemplate(item, itemIndex, template) {
    return `<li>${item.openfda.brand_name}
            ${item.openfda.route}
            ${item.openfda.generic_name}</li>`;
  }
